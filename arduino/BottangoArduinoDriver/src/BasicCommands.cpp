...
// not only does this make sense, but it breaks otherwise. I'd figure out why if I wanted it to work
        ...
        BottangoCore::serial.write(code);
        BottangoCore::serial.print(F(","));

        // true if accepting incoming commands, false if not
#ifndef USE_COMMAND_STREAM
        BottangoCore::serial.print(F("1"));
#else
        BottangoCore::serial.print(F("0"));
#endif
        BottangoCore::serial.write("\n");

        BottangoCore::serial.flush();
    ...
    void printOutputString(const char *targetOutput)
    {
        ...
        {
            BottangoCore::serial.print((char)pgm_read_byte(targetOutput));
            targetOutput++;
        }
    }
} // namespace BasicCommands
